...
var formattedText = '';
// Hiển thị văn bản 
function typeText(text_items) {

    var element = document.getElementById("typing-text");
    element.innerHTML = ''
    var textToType = text_items[Math.floor(Math.random()*text_items.length)];

    // Thay thế '\n' bằng thẻ HTML <br>
    formattedText = textToType.replace(/\n/g, '<br>');


    // Thời gian trễ giữa các ký tự (milliseconds)
    var delayBetweenCharacters = 100;   

    var index = 0;

    function addCharacter() {
        element.innerHTML += textToType[index];
        index++;

        if (index < textToType.length) {
            setTimeout(addCharacter, delayBetweenCharacters);
        } else {
            // Hiển thị toàn bộ nội dung sau khi đánh máy xong
            setTimeout(showWholeText, delayBetweenCharacters);
        }
    }
...
// Gọi hàm bắt đầu hiển thị 


function getText() {

    // Đường dẫn đến file JSON
    var jsonFilePath = './new_year.json';

    fetch("./new_year.json") 
        .then((res) => { 
        return res.json(); 
    }) 
    .then((data) => typeText(data)); 
